  ...
  methods: {
    ...
    async open() {
      ...
      this.$alert(`<img src="${url}" >`, "请使用微信扫码支付!", {
        ...
        cancelButtonText: "支付发生错误!",
        confirmButtonText: "已支付成功!",
        showClose: false,
        beforeClose: (action, instance, done) => {
          //action=>点击的类型 action 的值为'confirm', 'cancel'或'close',
      ...
      if (!this.timer) {
        ...
        this.timer = setInterval(async () => {
          let result = await this.$API.reqPayStatus(this.orderInfo.orderId);
          if (result.code == 200) {
            //支付成功
            //清除定时器,设置timer为null
            clearInterval(this.timer);
            this.timer = null;
            //保存code，防止用户直接点击支付成功而跳过支付流程
