  ...
  methods: {
    ...
    async addShopCart() {
      try {
        ...
        //成功要做的事情
        //进行路由跳转,并且把数据传递过去，像产品数量这些简单的数据直接用query传参即可，像skuInfo这种复杂的对象数据，
        //可以使用html5新增的存储方式，本地存储和会话存储来储存，本地存储适用于长时间存储，因为添加到购物车信息不需要长时间存储，使用用会话存储即可
        this.$router.push({
          name: 'addcartsuccess',
          query:{skuNum:this.skuNum}
        });
        //会话存储的值不能为对象，所以要先转换成字符串，然后读取的时候再转换回对象
